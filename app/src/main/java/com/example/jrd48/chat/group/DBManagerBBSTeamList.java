...
public class DBManagerBBSTeamList {
    private DBHelperTeamList helper;
    private SQLiteDatabase db;
    Context mContext;
    public final static String TABLE_NAME = "bbs_teamlist";

    public DBManagerBBSTeamList(Context context, String databaseName) {
    ...
    public DBManagerBBSTeamList(Context context, boolean bReadOnly, String databaseName) {
        helper = new DBHelperTeamList(context, databaseName);
        // 因为getWritableDatabase内部调用了mContext.openOrCreateDatabase(mName, 0,
        // mFactory);
        // 所以要确保context已初始化,我们可以把实例化DBManager的步骤放在Activity的onCreate里
