...
import com.example.jrd48.chat.group.DBManagerBBSTeamList;
...
//                i.putExtra("error_code", resp.getErrorCode());
                ...
                if (re.getErrorCode() == ProtoMessage.ErrorCode.OK_VALUE) {
                    ...
                    DBManagerBBSTeamList db = new DBManagerBBSTeamList(mContext, DBTableName.getTableName(mContext, DBHelperTeamList.NAME));
                    try {
                        db.add(list);
                    } catch (Exception e){
                        e.printStackTrace();
